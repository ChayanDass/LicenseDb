# ==============================================================================
# Makefile for LicenseDb Backend
# ==============================================================================
include .env
...
.PHONY: help dev build init prod test test-verbose test-coverage test-individual test-file test-func test-watch quick-test fmt lint clean deps create-migration migrate-up migrate-down migrate-force
...
help:
	...
	@echo "  make prod          - Run the compiled Go application in production"
	@echo "  make build         - Build the Go application"
	@echo "  make help          - Display available commands (default target)"
	@echo "  make init          - Run initial DB setup using ./scripts/init.sh"
	@echo ""
...
build:
	...
	go generate ./...
	go build -o ./laas ./cmd/laas
	@echo "Build complete: laas"

# Build the database 
init:
	@echo "Running Initial setup for Database"
	DB_HOST="$(DB_HOST)" \
	DB_PORT="$(DB_PORT)" \
	DB_NAME="$(DB_NAME)" \
	DB_USER="$(DB_USER)" \
